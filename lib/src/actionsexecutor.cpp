...
qint64 ActionsExecutor::getSub()
{
    ...
    throw std::exception("Empty subaction");
}
...
qint64 ActionsExecutor::getSum()
{
    ...
    throw std::exception("Empty sumaction");
}
...
qint64 ActionsExecutor::getXor()
{
    ...
    throw std::exception("Empty xoraction");
}
...
void ActionsExecutor::init()
{
    try {
        ...
        auto res = connect(filereader.get(), &Helper::FileReader::error, [&](QString errmsg) {throw std::exception(QString("FileReader error: %1").arg(errmsg).toLocal8Bit()); });
        if (!res)
            throw std::exception(u8"Connection failed");
